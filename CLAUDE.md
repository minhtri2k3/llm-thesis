...
- **Cohort study is feature-flagged.** `ENABLE_COHORT_STUDY` (default `false`) toggles the 4-Gemini single-blind controlled study (Indigo=2.5-flash, Crimson=2.5-pro, Emerald=3.1-flash-lite, Amber=3.1-pro-preview). When on, `POST /api/sessions` ignores client-supplied `preferred_model` and assigns one via the Latin square in `agent/cohort.py`. Each user does 4 sessions (one per model) before HTTP 409. Schema additions are additive only — flipping the flag off restores legacy behaviour byte-for-byte. The codename↔model map lives at `agent/cohort.py:CODENAME_TO_MODEL` (single source of truth). Cohort dashboard lives at `GET /api/analytics/cohort` (admin-protected, returns 503 when flag off).
