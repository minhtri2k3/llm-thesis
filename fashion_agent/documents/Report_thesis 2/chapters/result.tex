...
\textbf{Data freeze.} All aggregates in §5.2--§5.6 are computed from a snapshot of the production PostgreSQL database taken at \textbf{2026-05-10}. The cohort window is open, that is, no $t_{start}$ filter is applied; sessions from the earliest deployed traffic on 2026-04-20 up to and including 2026-05-10 are eligible. Because the deployed system continues to receive traffic, any reproduction with a later freeze date will return a larger cohort whose distribution may differ; the numbers below are therefore a point-in-time observation, not a stable historical record.
...
\midrule
Raw cohort (open window) & 61 & all rows of \texttt{user\_sessions}\\
Active cohort (had at least one impression) & 32 & inner join with \texttt{product\_impressions}\\
Rated cohort (had at least one rating) & 13 & inner join with \texttt{user\_ratings}\\
Demographic cohort (declared gender) & 58 & \texttt{gender IS NOT NULL}\\
Order-completing cohort (\texttt{ended\_by='order'}) & 11 & \texttt{user\_sessions.ended\_by} column\\
\bottomrule
\end{tabular}
\caption{Cohort counts at freeze date 2026-05-10 (open window, earliest session 2026-04-20)}
\label{tab:cohort_counts}
\end{table}

The active cohort (32 of 61, 52.5\%) is the most stable subset for behavioural metrics: it represents sessions in which the user actually saw search results. The rated cohort (13 of 61, 21.3\%) is small in absolute terms; §5.6 therefore reports its findings as anecdotal evidence and annotates every aggregate with the sample size $N=13$.
...
\midrule
\texttt{path1} & 36 & Primary text-to-image search path; the agent reasoning loop runs for these sessions\\
\texttt{path2} & 8  & Image upload path (single-stage cosine lookup, no agent reasoning); secondary in this chapter\\
\bottomrule
\end{tabular}
...
\bottomrule
\end{tabular}
\caption{Catalogue size and enrichment coverage at freeze date 2026-05-10}
...
\midrule
\texttt{intent}    & 209 & 2{,}003 & 120\\
\texttt{synthesis} & 95  & 879     & 219\\
\bottomrule
\end{tabular}
...
Two observations follow from Table~\ref{tab:call_name_distribution}:

\begin{enumerate}
    \item \textbf{Every turn invokes the intent classifier.} The 209 \texttt{intent} rows correspond to 209 turns, and they are the only mandatory call.
    \item \textbf{Synthesis is conditional, not mandatory.} Only 95 of the 209 turns proceeded to a synthesis call. The ratio $95/209 \approx 45.5\%$ is the fraction of user turns that produced product results worth synthesising; the remaining 54.5\% were either clarification responses (no synthesis call needed, the agent emits a templated question) or out-of-scope requests (refused without retrieval).
\end{enumerate}
...
\midrule
Sessions with at least one logged call & 48\\
Average input tokens per session & 10{,}458\\
Average output tokens per session & 957\\
Average total tokens per session & 11{,}415\\
\bottomrule
\end{tabular}
...
\midrule
\texttt{direct} & \texttt{gemini-2.5-flash} (``fixed'' label) & 48 & 209 & \$0.000142\\
\bottomrule
\end{tabular}
...
A representative turn costs approximately \$0.000142, that is, on the order of \$0.14 per one thousand turns at the published Gemini~2.5~Flash list price. The cost is dominated by the intent-classifier input (large system prompt) rather than by the synthesis output. All rows in the present cohort carry the legacy \texttt{orchestrator\_model='fixed'} label emitted by the deployed agent.
...
Impressions logged   & 444 & 14.8 impressions per funnel session (mean)\\
Clicks               & 93 & 20.95\% of impressions (CTR)\\
Cart-adds            & 24 & 25.81\% of clicks; 5.41\% of impressions\\
Will-buy intents     & 3 & 12.5\% of cart-adds\\
Not-for-me intents   & 2 & 8.33\% of cart-adds\\
Orders (session-end) & 12 & ---\\
\bottomrule
\end{tabular}
...
The click-through rate of 20.95\% is the strongest signal in the funnel: roughly one in five displayed items receives an explicit user click. The cart-add rate (5.41\% of impressions) drops by a factor of four from the click rate, which is consistent with users browsing freely and committing more selectively. The will-buy intent rate is small in absolute terms (only three events from 24 cart-adds), reflecting both genuine selectivity and the small absolute cohort; it is therefore reported but not used as a primary quality signal.
...
\midrule
Sessions in funnel & 8 & ---\\
Impressions logged & 54 & ---\\
Clicks             & 6 & 11.11\% of impressions\\
Cart-adds          & 2 & 33.3\% of clicks; 3.70\% of impressions\\
Will-buy intents   & 0 & ---\\
Orders             & 0 & ---\\
\bottomrule
\end{tabular}
...
PATH~2's small cohort (8 sessions) and complete absence of will-buy or order events mean the table is reported here for completeness only. PATH~2's funnel does not support any claim about agent quality, as discussed in §5.1 sample-balance.
...
\midrule
0 & 4  & 16.67\% --- cart-add via numeric selection without prior click\\
1 & 6  & top-of-list\\
2 & 4 & \\
3 & 2 & \\
4 & 2 & \\
5 & 3 & \\
6 & 3 & last visible position\\
\bottomrule
\end{tabular}
...
The share of \texttt{position=0} is 16.67\%, below the 20\% threshold beyond which the histogram would have been recomputed from the impressions join (the methodological design decision recorded in the change proposal). The distribution among positions 1--6 is tightly clustered with six cart-adds at position~1 (the modal position) and a gentle tail. This is consistent with the reranker putting useful items near the top, but the absolute counts (24 cart-adds in total) are too small to be claimed as statistically significant; the histogram is reported as suggestive evidence, not as a confirmatory test.


\section{Subjective Quality}
\label{sec:subjective_quality}

This section reports on the rating signals collected from \texttt{user\_ratings}. Because $N = 13$, every aggregate below is presented as anecdotal evidence and not as a statistical claim. The intent of this section is to triangulate the implicit signals in §5.5 with explicit user feedback.
...
\midrule
\texttt{rating\_overall}        & 4.31 & 0.95 & 13\\
\texttt{rating\_suggestions}    & 4.38 & 0.96 & 13\\
\texttt{rating\_conversation}   & 4.08 & 1.04 & 13\\
\bottomrule
\end{tabular}
\caption{User ratings on a 1--5 scale, $N=13$, freeze date 2026-05-10}
...
\begin{enumerate}
    \item Sample size $N=13$ falls below conventional thresholds for statistical inference; no test of mean differences is performed.
    \item Self-selection bias: users who rated may differ systematically from users who did not; the rated cohort is 21.3\% of the raw cohort.
    \item No paired design: there is no matched pre-deployment baseline against which to compare these scores. A mean of~4.31 on a 1--5 scale is favourable in absolute terms but cannot be argued as ``better than the previous system'' without comparable historical data.
\end{enumerate}
