...
\begin{lstlisting}[caption={Direct Routing Orchestrator --- agent/fashion\_agent.py}]
def _orchestrate_stream(
    ...
    # Step 2: Early exits for special intents (0 LLM)
    if intent == "out_of_scope":
        yield OrchestrateResult(intent="out_of_scope", clarification=msg)
        return
    if intent == "product_select":
        yield from _handle_product_select(...)
        return

    # Step 3: Resolve search query via slot gate (0 LLM)
    search_query, clarification, slots = _resolve_search_query(
        intent, intent_result, session_id, history, query)
    if clarification:
        yield OrchestrateResult(clarification=clarification)
        return

    # Step 4: Route & execute search (0 LLM)
    ...
    yield OrchestrateResult(
        ...
        products=products, reasoning=reasoning,
    )
\end{lstlisting}

\textbf{Clarification gate (confidence $< 0.6$):}
\begin{lstlisting}[caption={Confidence-based clarification --- agent/fashion\_agent.py}]
...
\begin{itemize}
    ...
    \item \textbf{A/B Testing Capabilities}: The \texttt{gender\_hint\_enabled} flag allows developers to toggle this feature for controlled experimentation and research easily.
\end{itemize}
