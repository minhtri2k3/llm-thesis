...
\texttt{agent/utils.py} & Shared helpers: supported-category ontology, exact/synonym/fuzzy category canonicalisation, unsupported-category suggestions, token accounting, language tagging, and small string utilities. \\
...
\texttt{search/query\_expansion.py} & Optional LLM expansion module (Algorithm~\ref{alg:expand_query}), gated by intent and query length. \\
...
For Python-only backend changes, the Compose service mounts the backend source tree into the \texttt{fashion-api} container. A container restart is therefore sufficient to reload modules such as \texttt{agent/fashion\_agent.py} and \texttt{agent/utils.py}; a full image rebuild is only required when dependencies, system packages, or Dockerfile layers change. This refresh path preserves both Postgres and Qdrant volumes and avoids repeating the expensive ingestion/indexing pipeline.
