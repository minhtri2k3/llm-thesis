...
Nine questions about the system are not answered by Chapter~5, and each is recorded here together with a concrete future-work direction.
...
\textbf{Slot vocabulary and ranked-slot weights are design-based, not learned.}
Two related slot-design choices in Section~3.3 are heuristic rather than empirical. First, the \emph{six-slot extraction vocabulary} $\{$category, color, fabric, fit, construction, aesthetic$\}$ mirrors the BM25-indexed metadata plus the four-axis offline captioning prompt; alternative vocabularies (for example, adding \texttt{season} or \texttt{gender} as gating slots, or substituting \texttt{pattern} for \texttt{construction}) were not benchmarked. Second, the \emph{ranked-slot weights} $\{$category{:}4, color{:}3, occasion{:}3, style{:}2$\}$ used by the clarification gate (Section~3.3.3, Table~\ref{tab:slot_weights}) and the per-intent thresholds were chosen by analysing retrieval-failure modes during prototyping rather than by optimising over a labelled gold set. As a consequence, the thesis cannot claim that either the slot vocabulary or the weight schedule is optimal in any quantitative sense. The future-work hook is to construct a held-out query set with relevance labels and run two ablations against it: a vocabulary ablation that varies the slot set, and a weight-learning procedure (for example, logistic regression over per-slot indicator features against retrieval success) so that both the vocabulary and the weight schedule can be derived empirically rather than asserted.
...
\textbf{The liked-items endorsement path is scaffolded but inactive.}
The \texttt{liked\_items} JSONB column on \texttt{user\_sessions} and the \texttt{add\_liked\_item} helper (Section~3.3.5, Algorithm~\ref{alg:add_liked_item}) are present in code, but no HTTP endpoint and no Flutter UI element triggers them; \texttt{get\_preferences} also ignores \texttt{liked\_items} and reads only \texttt{query\_history}. As a consequence, the only deployed user-endorsement signal is the \texttt{selected\_items} table written through the \texttt{product\_select} intent (Algorithm~\ref{alg:save_selected_items}). The future-work hook is to add an explicit \emph{like} action to the Flutter client, expose an HTTP endpoint that calls \texttt{add\_liked\_item}, and wire the resulting array back into \texttt{get\_preferences} so that explicit endorsement and selection-by-ordinal are both available as soft personalisation hints.

\textbf{Compound multi-product queries fall back to a generic clarification template rather than decomposing into sub-queries.}
When a user issues a compound query that references multiple distinct products in the same message --- for example, an English-equivalent message such as ``find me a blue shirt for a date and a pair of black comfortable pants for sports'', issued as a single chat turn --- the deployed system relies on the intent classifier to either drop confidence below the gate threshold or emit empty/conflicting category and color slots, both of which route the turn into the clarification gate (Section~3.3.3). The gate then selects the generic ``missing category + color'' clarification template, which asks the user to focus on a single product. This is a safe fallback (no silent product drop, no noisy mixed-result top-6) but it is not query decomposition: the system never splits the message into parallel sub-queries. Figure~\ref{fig:compound_query_clarification} captures the deployed behaviour on a real session: the compound input on the right elicits the generic single-slot prompt on the left rather than a compound-aware response that would acknowledge the two distinct items. The future-work hook is twofold. First, add a query-segmentation pass before intent classification (rule-based on coordinating conjunctions, or LLM-driven) that emits a list of sub-queries; the seven-stage retrieval would then run independently per sub-query and the synthesis prompt would be extended to compose outfit-bundle answers from disjoint result sets. Second, add a compound-aware clarification template that explicitly acknowledges the multiple items and offers the user the choice of which to retrieve first, instead of the current generic single-slot prompt.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{screenshot_compound_query_clarification.png}
\caption{Compound multi-product query falling back to the generic clarification template. The user message on the right requests both a blue shirt for a date and black sport pants in a single turn; the deployed agent responds (left) with the generic single-slot clarification template asking for clothing type and color (rendered in the session locale) rather than a compound-aware prompt. This is the safe-but-undecomposed behaviour discussed above.}
\label{fig:compound_query_clarification}
\end{figure}
...
Independent of the nine gaps above, two further directions are noted for completeness. They are not gaps in the present evidence but extensions of the system itself: scaling the catalogue from the current 5{,}096 items to a 15K--100K-item working set with a fine-tuned reranker; and adding visual try-on (for example IDM-VTON) on top of PATH~2 so that the visual lookup pipeline produces a virtual fitting rather than a similarity list. Both are out of scope for this thesis but follow naturally from the architecture documented in Chapter~3.
