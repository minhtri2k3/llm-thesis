...
\begin{table}[H]
\centering
\caption{Offline pipeline scripts (\texttt{fashion\_agent/pre\_processing/} and \texttt{fashion\_agent/indexing/}).}
...
4. ``product\_select'' --- User wants to SELECT/CHOOSE specific products from the most recently shown search results. Signals: numbers (1-6, ``\#2''), ordinals (``first'', ``third'', ``đầu tiên''), selection verbs (``I want'', ``lấy'', ``chọn'', ``pick'', ``I'll take''), or description references (``the quilted one'', ``cái áo navy''). ONLY classify as product\_select if the conversation history shows recent search results were displayed.

5. ``view\_selections'' --- User wants to view their saved/selected products (e.g., ``show my selections'', ``what did I pick?'', ``my picks'', ``xem các sản phẩm đã chọn'', ``ver mis selecciones'')
...
\begin{table}[H]
\centering
\caption{Postgres relations initialised at API startup.}
\label{tab:impl_pg_tables}
\small
\begin{tabular}{p{4.4cm} p{3.0cm} p{6.6cm}}
\toprule
\textbf{Relation} & \textbf{Primary key} & \textbf{Role} \\
\midrule
\texttt{user\_sessions} & \texttt{session\_id} & Per-session demographic context (user name, year of birth, gender), preferred model, and JSONB columns \texttt{liked\_items} and \texttt{query\_history} for soft preferences (Algorithm~\ref{alg:get_preferences}). \\
\texttt{conversation\_history} & \texttt{id} (BIGSERIAL) & Append-only message log with foreign key to \texttt{user\_sessions}; indexed on (\texttt{session\_id}, \texttt{created\_at}). \\
\texttt{selected\_items} & \texttt{id} & Confirmed selections with \texttt{position} and \texttt{path\_mode}; uniqueness on (\texttt{session\_id}, \texttt{image\_id}, \texttt{path\_mode}). \\
\texttt{user\_ratings} & \texttt{id} & Three 1--5 ratings (\texttt{rating\_overall}, \texttt{rating\_suggestions}, \texttt{rating\_conversation}) plus free-text feedback per session. \\
\texttt{llm\_token\_usage} & \texttt{id} & Per-call input / output token counts per model and call name; basis for the cost dashboards in Result. \\
\texttt{product\_impressions}, \texttt{product\_clicks}, \texttt{product\_intents} & \texttt{id} & Per-event behaviour log keyed by (\texttt{session\_id}, \texttt{image\_id}, \texttt{path\_mode}) with positions. \\
\texttt{user\_orders} & \texttt{id} & Simulated checkout (phone, address, \texttt{path\_mode}); writing a row also stamps \texttt{ended\_at} on the parent session. \\
\texttt{cart\_removals} & \texttt{id} & Cart-removal events, distinct from \texttt{selected\_items} so removal rate is computable. \\
\bottomrule
\end{tabular}
\end{table}

Connection management is handled by a module-level \texttt{psycopg2.pool.SimpleConnectionPool} configured with \texttt{minconn=2}, \texttt{maxconn=10}, \texttt{connect\_timeout=5}, and a server-side \texttt{lock\_timeout} of 8\,s applied through \texttt{options}. A lightweight \texttt{\_db\_conn()} context manager borrows and returns a connection per logical operation, keeping all explicit \texttt{conn.close()} calls out of the request handlers. Two soft-preference columns on \texttt{user\_sessions} --- \texttt{liked\_items} and \texttt{query\_history} --- use the JSONB type with default \texttt{[]}; this is what allows Algorithm~\ref{alg:add_liked_item} and Algorithm~\ref{alg:log_query_history} to be implemented as single \texttt{UPDATE} statements with \texttt{jsonb} concatenation.
