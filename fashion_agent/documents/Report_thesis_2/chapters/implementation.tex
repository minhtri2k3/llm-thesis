...
\small
\begin{tabular}{p{3.4cm} p{3.6cm} p{6.6cm}}
\toprule
\textbf{Component} & \textbf{Version (pinned / image)} & \textbf{Role} \\
\midrule
Python runtime & 3.11 (slim base image) & Hosts the FastAPI process, the agent, the search engine, and the model loaders. \\
...
PostgreSQL & 16-alpine (\texttt{postgres:16-alpine}) & Source of truth for sessions, conversation history, selections, ratings, token usage, impressions, clicks, intents, orders. \\
Qdrant & \texttt{qdrant/qdrant:latest} & Vector store for image and text embeddings; collection \texttt{fashion\_products}. \\
...
\texttt{open-clip-torch} + Marqo-FashionSigLIP & $\geq 2.24$ / 768-d & Cross-modal encoder used by Algorithm~\ref{alg:fashionsiglip} to project images and queries into a shared cosine space. \\
\texttt{transformers} + BGE Reranker v2-m3 & $\geq 4.36$ / 568\,M params & Cross-encoder used by Algorithm~\ref{alg:rerank_blend} to score the fused candidate set. \\
\texttt{torch} & $\geq 2.1$ & Tensor backend for both encoders; chooses the device at first use. \\
Gemini 2.5 Flash via \texttt{google-generativeai} & $\geq 0.3$ & The single LLM client wired into intent classification (Algorithm~\ref{alg:intent_class}), query expansion (Algorithm~\ref{alg:expand_query}), and synthesis (Algorithm~\ref{alg:synthesize_response}). \\
...
ngrok agent & \texttt{ngrok/ngrok:latest} & Public-URL tunnel; binds a reserved \texttt{*.ngrok-free.app} subdomain to \texttt{clothie-web:80}. \\
\bottomrule
\end{tabular}
\end{table}
...
Every prompt template lives in a single Python module (\texttt{agent/prompts.py}); inlining prompts elsewhere is forbidden by an in-file warning so that every template remains reviewable in one place. Table~\ref{tab:impl_prompt_artefacts} lists the artefacts and the JSON shape each call is expected to return.
...
\small
\begin{tabular}{p{3.0cm} p{4.6cm} p{4.0cm} p{2.6cm}}
\toprule
\textbf{Name} & \textbf{Purpose} & \textbf{Expected JSON shape} & \textbf{Fallback on parse failure} \\
\midrule
\texttt{INTENT\_PROMPT} & Single-call intent classification + six-slot extraction + selection-number extraction; few-shot, multilingual. & \texttt{intent}, \texttt{confidence}, \texttt{filters}, \texttt{refined\_query}, six \texttt{slot\_*}, \texttt{selected\_numbers}. & Treat as \texttt{unclear}, route to deterministic clarification (Algorithm~\ref{alg:build_clarification}). \\
\texttt{SYNTHESIS\_PROMPT} & Non-streamed final answer with optional styling tip; locked to detected language. & \texttt{answer}, \texttt{styling\_suggestion}. & Use raw text as \texttt{answer}, leave \texttt{styling\_suggestion} empty. \\
\texttt{STREAM\_SYNTHESIS\_PROMPT} & Streaming variant; emits plain text and an inline ``Styling tip:'' marker rather than JSON. & Plain text; \texttt{token} SSE events. & Plain text already; nothing to recover from. \\
\texttt{EXPANSION\_PROMPT} & Generates short paraphrases of the user query for BM25 recall (Algorithm~\ref{alg:expand_query}). & JSON array of strings. & Use only the original query; skip expansion. \\
\texttt{CLARIFICATION\_PROMPT} & LLM-driven clarification used only when the deterministic gate cannot construct one. & \texttt{question}. & Use the per-language \texttt{FALLBACK\_QUESTION}. \\
\texttt{OUT\_OF\_SCOPE\_RESPONSE}, \texttt{UNSUPPORTED\_CATEGORY\_RESPONSE} & Static refusal / redirect messages keyed by language. & --- (no LLM call). & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\label{tab:impl_env}
\small
\begin{tabular}{p{4.0cm} p{2.4cm} p{7.0cm}}
\toprule
\textbf{Variable} & \textbf{Consumer} & \textbf{Default / Effect when absent} \\
\midrule
\texttt{PG\_PASSWORD} & \texttt{postgres}, \texttt{fashion-api} & Required; Compose refuses to start without it. \\
\texttt{PGHOST} / \texttt{PGPORT} / \texttt{PGDATABASE} / \texttt{PGUSER} & \texttt{fashion-api} & Defaults: \texttt{postgres}, \texttt{5432}, \texttt{fashion\_rag}, \texttt{fashion\_user}. \\
\texttt{QDRANT\_HOST} / \texttt{QDRANT\_PORT} & \texttt{fashion-api} & Defaults: \texttt{qdrant}, \texttt{6333}. \\
...
\texttt{ENABLE\_PATH2\_IMAGE\_SEARCH} & \texttt{fashion-api} & Default \texttt{false}; PATH 2 endpoints return 503 until set. \\
\texttt{PATH2\_MAX\_UPLOAD\_BYTES} / \texttt{PATH2\_MAX\_TOP\_K} & \texttt{fashion-api} & Defaults: 5\,MiB and 20; bound the PATH 2 endpoint inputs. \\
\texttt{SEARCH\_CONFIDENCE\_THRESHOLD} & \texttt{fashion-api} & Default 0.75; threshold of the slot-readiness gate (Algorithm~\ref{alg:slot_readiness}). \\
\texttt{DATASET\_IMAGES\_HOST\_PATH} & \texttt{fashion-api} & Bound read-only at \texttt{/app/dataset\_images}; falls back to \texttt{./images} when unset. \\
\texttt{HF\_HOME} / \texttt{TRANSFORMERS\_CACHE} & \texttt{fashion-api} & Pinned to \texttt{/app/models} so encoder weights cache to a host-mounted directory. \\
