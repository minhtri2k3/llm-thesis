...
\toprule
\textbf{Evidence Signal} & \textbf{Source Table or View} & \textbf{Where it Appears}\\
\midrule
Session inventory & \texttt{user\_sessions}, \texttt{session\_path\_funnel\_summary} & §5.2 cohort and integrity\\
Knowledge-base content & \texttt{fashion\_items}, \texttt{fashion\_item\_enrichment}, \texttt{fashion\_processing\_logs} & §5.3 knowledge-base quality\\
LLM call accounting & \texttt{llm\_token\_usage}, \texttt{session\_token\_summary}, \texttt{mode\_cost\_summary} & §5.4 LLM call discipline\\
User behaviour funnel & \texttt{product\_impressions}, \texttt{product\_clicks}, \texttt{selected\_items}, \texttt{cart\_removals}, \texttt{product\_intents}, \texttt{user\_orders}, \texttt{session\_path\_funnel\_summary} & §5.5 implicit relevance\\
Subjective ratings & \texttt{user\_ratings} & §5.6 subjective quality\\
\bottomrule
\end{tabular}
\caption{Canonical evidence sources used for the empirical sections}
...
A metric is treated as trustworthy only when it passes the checks in Table~\texttt{\ref{tab:data_validity_checks}}; otherwise the surrounding prose explicitly weakens the claim. Sample-size caveats are repeated in every subsection where the post-filter cohort is small.
...
\midrule
\texttt{path1} & 27 & Primary text-to-image search path; the agent reasoning loop runs for these sessions\\
...
\label{subsec:session_lifecycle}

Of the 53 raw sessions, 8 (15.1\%) ended explicitly through the simulated checkout flow (\texttt{ended\_by='order'}). The remainder either ended via rating or were never explicitly closed by the user; the schema does not distinguish ``timeout'' from ``user closed the tab'' beyond the absence of \texttt{ended\_at}. Orders are reported here only as a session-end signal --- not as a quality metric, because the simulated checkout records phone and address but does not represent a real purchase.
...
\subsection{Processing Log Coverage}
\label{subsec:processing_log_coverage}

The \texttt{fashion\_processing\_logs} table is empty in the present snapshot. This is a data-quality observation rather than a claim about enrichment success: the table is populated only when a processing pass writes step-by-step records, and the cohort window did not include such a pass. The fact that 100\% of items have non-empty captions and colors (Table~\texttt{\ref{tab:enrichment_coverage}}) indicates that enrichment has run to completion at some point before the freeze date; the per-step trace simply is not available for that historical run.
...
Three observations follow from Table~\texttt{\ref{tab:call_name_distribution}}:
...
\begin{enumerate}
    ...
    \item \textbf{The query-expansion call is never recorded.} The short-query gate at six words (Algorithm~\texttt{\ref{alg:expand_query}}) suppressed expansion in every observed turn; \texttt{call\_name='expansion'} has zero rows. In practice the deployed cohort always reached search with the user's original query.
\end{enumerate}
...
\subsection{Cart-Removal Rate}
\label{subsec:cart_removal_rate}

The \texttt{cart\_removals} table contains zero rows in the present snapshot. The interpretation has two non-exclusive components: (a) users in the active cohort did not change their mind about any item they added; (b) the present version of the frontend may not surface a one-tap cart-removal action prominently, making the metric a UI-design proxy rather than a pure quality signal. With the data we have, no conclusion is drawn from this number; it is reported because it is part of the canonical evidence-source list in Table~\ref{tab:canonical_evidence_sources}.
